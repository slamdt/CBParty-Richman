...
void Tween::updateHandler()
{
    ...
    if(m_fFrameTweenEasing != SP_TWEEN_EASING_MAX)
	{
...
float Tween::updateFrameData(float _currentPrecent, bool _activeFrame)
{
	...
	/*
     *  if frame tween easing equal to SP_TWEEN_EASING_MAX, then it will not do tween.
     *  -1  :   fade out
     *  0   :   line
     *  1   :   fade in
     *  2   :   fade in and out
     */
 	...
 	if ( m_fFrameTweenEasing != SP_TWEEN_EASING_MAX)
     {
         _tweenEasing = (m_iTweenEasing == SP_TWEEN_EASING_MAX) ? m_fFrameTweenEasing : m_iTweenEasing;
         if (_tweenEasing)
         {
